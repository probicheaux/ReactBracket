...
export const loginUser = async (username: string, password: string) => {
    const resp = await postRequest(`${apiUnversionedHost}/user/login`, { username, password });
    return (resp).token;
}

export const registerUser = async (email: string, username: string, password: string) => {
    const resp = await postRequest(`${apiUnversionedHost}/user/register-from-email/`, { email, username, password });
    return resp.data
}
